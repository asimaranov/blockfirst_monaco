...
import {
  ...
  type TextContents,
  type TextModels,
  verifyUrlOrCreateDataUrl,
} from './editorApp';
import {
  type LanguageClientConfig,
  LanguageClientWrapper,
} from './languageClientWrapper';

export type { TextContents, TextModels, ModelRefs };
...
export class MonacoEditorLanguageClientWrapper {
  ...
  private extensionRegisterResults: Map<string, RegisterExtensionResult> =
    new Map();
  ...
  async init(wrapperConfig: WrapperConfig) {
    ...
    if (
      (editorAppConfig?.useDiffEditor ?? false) &&
      !editorAppConfig?.codeResources?.original
    ) {
      throw new Error(
        `Use diff editor was used without a valid config. code: ${editorAppConfig?.codeResources?.modified} codeOriginal: ${editorAppConfig?.codeResources?.original}`
      );
    }

    const viewServiceType =
      wrapperConfig.vscodeApiConfig?.viewsConfig?.viewServiceType ??
      'EditorService';
    if (
      wrapperConfig.$type === 'classic' &&
      (viewServiceType === 'ViewsService' ||
        viewServiceType === 'WorkspaceService')
    ) {
      throw new Error(
        `View Service Type "${viewServiceType}" cannot be used with classic configuration.`
      );
    }
    ...
    if (wrapperConfig.automaticallyDispose ?? true) {
      ...
      if (this.wrapperConfig !== undefined) {
        throw new Error(
          'You configured the wrapper to not automatically dispose on init, but did not dispose manually. Please call dispose first if you want to re-start.'
        );
      }
    }
    try {
      ...
      if (
        typeof wrapperConfig.editorAppConfig?.monacoWorkerFactory === 'function'
      ) {
        wrapperConfig.editorAppConfig.monacoWorkerFactory(this.logger);
      }
      if (
        !(
          wrapperConfig.vscodeApiConfig?.vscodeApiInitPerformExternally === true
        )
      ) {
      ...
      this.editorApp = new EditorApp(
        this.wrapperConfig.$type,
        this.wrapperConfig.editorAppConfig,
        this.logger
      );
      console.log('init 12');
  ...
  initLanguageClients() {
    const lccEntries = Object.entries(
      this.wrapperConfig?.languageClientConfigs?.configs ?? {}
    );
  ...
  async initExtensions() {
    ...
    if (this.wrapperConfig?.extensions) {
      ...
      console.log('initExtensions ###4');

      for (const extensionConfig of extensions ?? []) {
        console.log(
          'initExtensions ###5',
          extensionIds,
          `${extensionConfig.config.publisher}.${extensionConfig.config.name}`
        );
        ...
        if (
          ...
          console.log('initExtensions ###6');

          const manifest = extensionConfig.config as IExtensionManifest;
          const extRegResult = registerExtension(
            manifest,
            ExtensionHostKind.LocalProcess
          );
          this.extensionRegisterResults.set(manifest.name, extRegResult);

          console.log('extRegResult', await extRegResult.isEnabled());
          if (
            extensionConfig.filesOrContents &&
            Object.hasOwn(extRegResult, 'registerFileUrl')
          ) {
            for (const entry of extensionConfig.filesOrContents) {
              this.disposableStoreExtensions?.add(
                extRegResult.registerFileUrl(
                  entry[0],
                  verifyUrlOrCreateDataUrl(entry[1])
                )
              );
            }
          }
          console.log('Pushing extension', extensionConfig.config.name);
          allPromises.push(extRegResult.whenReady());
        }
      }
      console.log('initExtensions 3', allPromises);
      // await Promise.all(allPromises);
      console.log('initExtensions 4');
      console.log(
        'Updated extensions',
        getExtensionManifests().map((ext) => ext.identifier.id)
      );
    }
    console.log('initExtensions 5');
  }
  ...
  async start(htmlContainer?: HTMLElement) {
    ...
    if (this.wrapperConfig === undefined) {
      throw new Error(
        'No init was performed. Please call init() before start()'
      );
    }
    this.markStarting();
    try {
      const viewServiceType =
        this.wrapperConfig.vscodeApiConfig?.viewsConfig?.viewServiceType;
      if (
        ...
        const html =
          htmlContainer === undefined
            ? this.wrapperConfig.htmlContainer
            : htmlContainer;
        if (html === undefined) {
          throw new Error(
            'No html container provided. Unable to start monaco-editor.'
          );
        } else {
        ...
        this.logger.info(
          'No EditorService configured. monaco-editor will not be started.'
        );
      }

      if (
        this.wrapperConfig.languageClientConfigs?.automaticallyStart ??
        true
      ) {
  ...
  getLanguageClientWrapper(
    languageId: string
  ): LanguageClientWrapper | undefined {
  ...
  async dispose() {
    ...
    console.log('dispose 8');

    try {
      if (
        this.wrapperConfig?.languageClientConfigs?.automaticallyDispose ??
        true
      ) {
  ...
  async disposeLanguageClients() {
    const disposeWorker =
      this.wrapperConfig?.languageClientConfigs?.automaticallyDisposeWorkers ??
      false;
    const allPromises: Array<Promise<void>> = [];
    for (const lcw of this.languageClientWrappers.values()) {
      if (lcw.haveLanguageClient()) {
