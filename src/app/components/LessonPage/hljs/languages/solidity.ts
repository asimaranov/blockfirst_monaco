...
function hljsDefineSolidity(hljs: any) {
  ...
  var sizePrecisionPairs = numSizes.flatMap(function (size) {
  ...
  var SOL_SPECIAL_PARAMETERS_PARTIAL_RE =
  ...
  function makeBuiltinProps(obj: any, props: any) {
    return {
      begin:
        (isNegativeLookbehindAvailable() ? '(?<!\\$)\\b' : '\\b') +
        obj +
