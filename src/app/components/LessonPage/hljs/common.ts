...
if (isNegativeLookbehindAvailable()) {
  SOL_NUMBER_RE = new RegExp(
    SOL_NUMBER_RE.source.replace(/\\b/g, '(?<!\\$)\\b')
  );
}
...
function solAposStringMode(hljs: any) {
  return hljs.inherit(
...
function solQuoteStringMode(hljs: any) {
  return hljs.inherit(
...
function baseAssembly(hljs: any) {
