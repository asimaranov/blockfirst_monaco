...
import { getStreakAction, updateUserStreak } from '~/server/api/routers/userData';
import { CourseSections } from '../../LessonSidebar/CourseSections';
...
async function Cover({
  ...
  const notifications = await api.notifications.getUnreadCount();
  ...
  const streakAndXpServer = await api.userData.getStreakAndXp();
  ...
  const stats: Stat[] = [
    {
      icon: FireIcon,
      alt: 'Fire',
      value: session?.user ? streakAndXpServer?.streak.count || 0 : 0,
      label: 'Стрик',
    },
    {
      icon: SparklesIcon,
      alt: 'Sparkles',
      value: session?.user ? streakAndXpServer?.xp.total || 0 : 0,
      label: 'XP',
    },
  ];

  let userData = await api.userData.getUserData();
  const streakData = getStreakAction(streakAndXpServer.lastLoginDate || Date.now());
  ...
  return (
    <>
      <div className="relative sm:hidden">
        ...
        />
        {(userData?.plan === 'free' || userData?.userId == null) && (
        ...
        <div className="relative z-10 flex h-full flex-col justify-between px-5 py-8 sm:px-16">
          ...
          <div className="hidden items-start justify-between sm:flex">
            ...
            <div className="flex items-center gap-8">
              ...
              <StreakAndXp
                session={session}
                streakAndXpServer={streakAndXpServer}
              />
              ...
              <div className="flex items-center gap-3">
                ...
                {/* Icon Buttons */}
                {<ClientActions className={`border-foreground bg-foreground hover:bg-background group flex cursor-pointer items-center justify-center rounded-full border p-3 transition-colors duration-100`} />}
              </div>
            </div>
          </div>
          ...
          <div className="mt-auto flex flex-col gap-8">
            <div className="flex items-center gap-3 sm:hidden">
              ...
              <div className="ml-auto flex">
                <ClientActions className={`border-foreground bg-foreground hover:bg-background group h-10 w-10 shrink-0 cursor-pointer rounded-full border p-3 transition-colors duration-100`} />
              </div>
            </div>
